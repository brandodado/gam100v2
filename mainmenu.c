...
#include "tutorial.h"   // For Tutorial state
...
#define BUTTON_SPACING 40.0f // Vertical space between buttons
...
static void DrawButton(const char* text, float x, float y, float w, float h, int is_hovered)
{
    ...
    CP_Font_Set(menu_font); // Ensure menu font is used
    CP_Settings_TextSize(32);
    CP_Settings_TextAlignment(CP_TEXT_ALIGN_H_CENTER, CP_TEXT_ALIGN_V_MIDDLE);
    CP_Font_DrawText(text, x, y + 2.0f); // Small offset for better vertical centering
}
...
void Main_Menu_Update(void)
{
    ...
    {
        if (hover_start) {
            // --- UPDATED: Always start a fresh game ---
...
{
    // --- UPDATED: Free the menu font ---
    if (menu_font) {
        CP_Font_Free(menu_font);
        menu_font = 0;
    }
}